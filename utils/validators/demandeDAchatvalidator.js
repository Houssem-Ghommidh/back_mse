...
exports.createdemandedachatValidator=[
    ...
    check('isConfirmed').notEmpty().withMessage('isConfirmed required').isBoolean().withMessage('isConfirmed must be True or False'), 
    check('description').notEmpty().withMessage('description required').isString().withMessage('description must be String'),
    check('id_produit').isMongoId().withMessage('Invalid projet id ')
    .custom((id_produit) =>
